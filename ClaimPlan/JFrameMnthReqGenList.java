    ...
    void initUserDet() {
        ...
        try {
            ...
            if ((requestDays < -5)) {
                ...
                JOptionPane.showMessageDialog(this, "You have Perdiem request/s that have not been acquitted for " + requestDays
                        + " days. Please acquit outstanding Perdiem request/s first");
    ...
    void checkOutstandingRequest() {
        try {
            ...
            ResultSet r = st.executeQuery("select  min(days) from (SELECT ref_num,  \n"
                    ...
                    + " group by REF_NUM) a\n"
                    + " where  REF_NUM in (SELECT REF_NUM FROM [ClaimsAppSysZvandiri].[dbo].[ClaimsWFActTab] where DOC_STATUS = 'HeadApprove')\n");
            //       + "group by REF_NUM");
...
//            
    ...
    void fetchdata() {
        try {
            ...
            st.executeQuery("SELECT distinct a.PLAN_REF_NUM,DISTRICT,Wk1_PLAN_START_DATE,Wk1_PLAN_END_DATE,\n"
                    ...
                    + "where PROVINCE = '" + provNam + "' and a.ACT_REC_STA = 'C' and b.DOC_STATUS = 'Approved'"
                    + " and a.plan_ref_num in (SELECT PLAN_REF_NUM FROM [ClaimsAppSysZvandiri].[dbo].[PlanUsrRecTab] where STATUS = 'A' and EMP_NAM = '" + jLabelGenLogNam.getText() + "')order by 1");
    ...
    private void initComponents() {
        ...
        jMenuMonthlyPlan.add(jSeparator33);
